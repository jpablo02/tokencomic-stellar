...
impl SorobanNFT {
    ...
    pub fn initialize(env: Env, name: Bytes, symbol: Bytes, uri: Bytes) {
        ...
        env.storage()
            .persistent()
            .set(&DataKey::TokenCount, &0i128); // Initialize token count
    }
    ...
    pub fn name(env: Env) -> Bytes {
        env.storage()
            .persistent()
            .get(&DataKey::Name)
            .unwrap_or_else(|| Bytes::from_slice(&env, b""))
    }
    ...
    pub fn is_approved_for_all(env: Env, owner: Address, operator: Address) -> bool {
        env.storage()
            .persistent()
            .get::<_, bool>(&DataKey::OperatorApproval(owner, operator))
            .unwrap_or(false)
    }
    ...
    pub fn transfer_from(
        ...
        env.storage()
            .persistent()
            .remove(&DataKey::Approvals(token_id));
    ...
    pub fn mint(env: Env, to: Address) {
        ...
        let already_minted: bool = env
            .storage()
            .persistent()
            .get(&DataKey::Minted(to.clone()))
            .unwrap_or(false);
        assert!(!already_minted, "Address has already minted a token");
        env.storage()
            .persistent()
            .set(&DataKey::Minted(to.clone()), &true);
        token_count += 1;
        env.storage()
            .persistent()
        ...
        env.storage()
            .persistent()
            .set(&DataKey::Owner(token_count), &to);

        env.events()
            .publish((symbol_short!("Mint"),), (to, token_count));
    }
}
